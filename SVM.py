# -*- coding: utf-8 -*-
"""
Created on Sun Jan 03 09:13:23 2016
...
def calcKernelMatrix(train_x, kernelOption):
	numSamples = train_x.shape[0]
      #print type(zeros(numSamples, numSamples))
...
def testSVM(svm, test_x, test_y):
    ...
    for i in xrange(numTestSamples):
        ...
        resultLabels.append(sign(predict))
        if sign(predict) == sign(test_y[i]):
            matchCount += 1
    accuracy = float(matchCount) / numTestSamples
    resultLabels = dData.decastLabes(resultLabels)
    return accuracy, resultLabels

#获得SVM的误差矩阵
def calSVMError(svm, test_x, test_y, labelTypes):
    test_x = mat(test_x)
    test_y = mat(test_y)
    numTestSamples = test_x.shape[0]
    supportVectorsIndex = nonzero(svm.alphas.A > 0)[0]
    ...
    supportVectorAlphas = svm.alphas[supportVectorsIndex]
    #matchCount = 0
    labels = array(test_y.T)
    labels = list(labels[0])
    labels = dData.decastLabes(labels)
    #print labels
    errorPros = [[0]*len(labelTypes)]*len(labelTypes)
    resultLabels = []
    for i in xrange(numTestSamples):
        kernelValue = calcKernelValue(supportVectors, test_x[i, :], svm.kernelOpt)
        predict = kernelValue.T * multiply(supportVectorLabels, supportVectorAlphas) + svm.b
    ...
    resultLabels = dData.decastLabes(resultLabels)
    #print labels
    for index, label in enumerate(resultLabels):
        errorPros[label][int(labels[i])] += 1
        
    errorPros = list(array(errorPros)/len(test_y))
    #accuracy = float(matchCount) / numTestSamples
    return errorPros
